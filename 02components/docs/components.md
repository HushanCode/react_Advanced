...
对于类组件的执行，是在`react-reconciler/src/ReactFiberClassComponent.js`中：**实例化组件，得到组件实例 instance**
...
对于函数组件的执行，是在`react-reconciler/src/ReactFiberHooks.js`中 **执行我们的函数组件，得到 return 返回的 React.element对象**
...
> Demo: react_Advanced/02components/demo/Propsdemo
